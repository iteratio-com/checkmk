...
<template>
  ...
  >
    ...
    >
      <template #header>
        <CmkIcon
          :name="item.header.icon.name"
          class="demo-cmk-accordion-dev__header-icon"
        ></CmkIcon>
        <span class="demo-cmk-accordion-dev__header-title">
          {{ item.header.title }}
          <CmkAccordionItemStateIndicator :value="item.id"></CmkAccordionItemStateIndicator>
        </span>
      </template>
      <template #content>
        <div v-if="item.id === 'item-1'" class="demo-cmk-accordion-dev__additional-div">
...
<style scoped>
.demo-cmk-accordion-dev__header-icon {
  margin-right: 16px;
}

.demo-cmk-accordion-dev__header-title {
  margin: 0;
}

.demo-cmk-accordion-dev__additional-div {
  background: red;
  padding: 16px;
  margin: 0 0 16px;
}
