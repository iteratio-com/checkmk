...
def _create_sns_limits(n_std_topics: int, n_fifo_topics: int, n_subs: int) -> SNSLimits:
    ...
    topics_fetcher = SNSTopicsFetcher(fake_sns_client, fake_tagging_client, "region", config)
    return SNSLimits(
...
def get_sns_sections() -> SNSSections:
    def _create_sns_sections(
        ...
        sns_summary = SNSSummary(fake_sns_client, region, config, sns_topics_fetcher, distributor)
        sns_sms = SNSSMS(fake_cloudwatch_client, region, config)
        sns = SNS(fake_cloudwatch_client, region, config)
