...
class OutdatedEntryFactory(EntryFactory):
    ...
    def published_parsed(cls) -> time.struct_time:
        return _random_time(
            oldest=1.0,
            newest=(CURRENT_TIME - aws_status._IGNORE_ENTRIES_OLDER_THAN).timestamp() - 1.0,  # noqa: SLF001
        )
...
def test__check_aws_status_no_issues() -> None:
    ...
    check_result = list(aws_status._check_aws_status(CURRENT_TIME, "Global", rss_feed))  # noqa: SLF001
    assert [aws_status._NO_ISSUES] == check_result  # noqa: SLF001
...
def test__restrict_to_region() -> None:
    ...
    us_east_entries = aws_status._restrict_to_region(entries, "US East (N. Virginia)")  # noqa: SLF001
...
def test__sort_newest_entry_first() -> None:
    ...
    sorted_entries = aws_status._sort_newest_entry_first(entries)  # noqa: SLF001
    assert [time.mktime(e.published_parsed) for e in sorted_entries] == [3.0, 2.0, 1.0]


def test__obtain_recent_entries() -> None:
    entries = aws_status._sort_newest_entry_first(RecentEntryFactory.batch(size=3))  # noqa: SLF001
    recent_entries = aws_status._obtain_recent_entries(CURRENT_TIME, entries)  # noqa: SLF001
...
def test__group_by_service_identifier_single() -> None:
    ...
    groups = aws_status._group_by_service_identifier(entries)  # noqa: SLF001
...
def test__group_by_service_identifier_two() -> None:
    ...
    entries = aws_status._sort_newest_entry_first(  # noqa: SLF001
    ...
    assert any(group == aws_status._sort_newest_entry_first(group) for group in groups)  # noqa: SLF001
...
def test__check_aws_status_for_service() -> None:
    ...
    # Act
    title_result, *entry_results = aws_status._check_aws_status_for_service(entries)  # noqa: SLF001
    # Assert
    assert title_result == v2.Result(
        state=aws_status._state_from_entry(newest_entry),  # noqa: SLF001
        summary=newest_entry.title,
    )
...
def test__state_from_entry(title: str, expected_state: v2.State) -> None:
    ...
    assert expected_state == aws_status._state_from_entry(entry)  # noqa: SLF001
