...
def unacknowledged_incompatible_werks() -> list[WerkV3]:
...
class ChangeLogPage(Page):
    ...
    def page(self, ctx: PageContext) -> PageResult:
        breadcrumb = make_simple_page_breadcrumb(
            main_menu_registry.menu_help(), _("Change log (Werks)")
        )
...
def acknowledge_werk(werk: WerkV3) -> None:
    acknowledge_werks([werk])


def acknowledge_werks(werks: Iterable[WerkV3], check_permission: bool = True) -> None:
    if check_permission:
...
def get_date_formatted(werk: WerkV3) -> str:
...
_SORT_AND_GROUP: dict[
    str | None,
    tuple[
        Callable[[Iterator[WerkV3]], list[WerkV3]],
        Callable[[WerkV3], None | str],
    ],
] = {
...
def render_werks_table_row(
    request: Request, table: Table, translator: werks_utils.WerkTranslator, werk: WerkV3
) -> None:
    table.row()
...
def _to_ternary_compatibility(werk: WerkV3) -> str:
    if werk.compatible == Compatibility.NOT_COMPATIBLE:
...
def werk_matches_options(werk: WerkV3, werk_table_options: WerkTableOptions) -> bool:
...
def render_werk_id(werk: WerkV3) -> str:
    return "#%04d" % werk.id


def render_werk_link(request: Request, werk: WerkV3) -> HTML:
...
def render_werk_title(request: Request, werk: WerkV3) -> HTML:
    title = werk.title
