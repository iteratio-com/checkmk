...
def check_cisco_ucs_lun(section: StringTable) -> CheckResult:
    ...
    yield Result(state=State.OK, summary="Size: %s" % size_readable)
    yield Result(state=State(mode_state), summary="Mode: %s" % mode_state_readable)
...
snmp_section_cisco_ucs_lun = SimpleSNMPSection(
    name="cisco_ucs_lun",
    detect=DETECT,
    fetch=SNMPTree(
