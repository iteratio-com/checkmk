...
# Non-free binaries (require non-community edition)
NONFREE_BINARIES: Sequence[BinarySmoke] = [
    BinarySmoke(
...
@pytest.mark.parametrize(
    "binary",
    (pytest.param(b, id=b.name) for b in NONFREE_BINARIES),
)
def test_nonfree_binary_smoke(binary: BinarySmoke, site: Site) -> None:
    """Run non-free binary in site context and verify it produces expected output."""
    p = site.run(binary.cmd_line(site), check=False)
    assert re.search(binary.expected_stdout, p.stdout, re.DOTALL)
    assert re.search(binary.expected_stderr, p.stderr, re.DOTALL)
