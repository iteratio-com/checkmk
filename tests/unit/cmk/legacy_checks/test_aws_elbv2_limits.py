...
def test_discover_aws_elbv2_limits(
    ...
    result = list(discover_aws_elbv2_limits(parsed_section))
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [['[["application_load_balancers",', '"TITLE",', "10,", "1,", '"REGION"]]']],
            [
                (0, "No levels reached", [("aws_elbv2_application_load_balancers", 1)]),
                (0, "\nTITLE: 1 (of max. 10)"),
            ],
        ),
    ],
...
def test_check_aws_elbv2_limits(
    ...
    parsed_section = parse_aws_limits_generic(info)
    result = list(check_aws_elbv2_limits(item, params, parsed_section))
