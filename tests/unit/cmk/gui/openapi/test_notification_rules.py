...
def test_forbid_explicit_email_addresses_in_a_create_notification_request_in_cloud(
    ...
    """Test that explicit_email_addresses field is forbidden in CLOUD edition."""
    monkeypatch.setattr(
...
def test_explicit_email_addresses_disabled_in_cloud(
    clients: ClientRegistry,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.parametrize("edition", [e for e in version.Edition if e != version.Edition.CLOUD])
def test_explicit_email_addresses_allowed_in_non_cloud_editions(
    clients: ClientRegistry,
    edition: version.Edition,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that explicit_email_addresses field is allowed in all non-CLOUD editions."""
    monkeypatch.setattr(
        "cmk.gui.openapi.endpoints.notification_rules.request_schemas.version.edition",
        lambda _: edition,
    )
