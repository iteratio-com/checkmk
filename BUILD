...
write_file(
    ...
    content = [
        ...
        "    # cmk-ccc contains version.py which is generated by bazel",
        "    # that's why we need to include the built wheel from bazel",
        "    sys.path = [",
        "        str(p)",
        "        for p in sorted(from_path.glob('packages/*'))",
        "        if p.joinpath('pyproject.toml').exists() and p.name != 'cmk-ccc'",
        "    ] + sys.path",
        "",
